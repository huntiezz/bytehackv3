...
import { Eye, Settings, MessageSquare, MessageCircle, Clock } from "lucide-react";
    ...
    return (
        <div className="min-h-screen bg-black text-zinc-100 flex justify-center py-12 px-4 font-sans selection:bg-white/20">
            ...
            <div className="w-full max-w-[920px] space-y-6">
                ...
                <div className="bg-[#050505] border border-zinc-900/50 rounded-[32px] p-8 min-h-[400px]">
                    <Tabs defaultValue="activity" className="w-full">
                        ...
                        <TabsContent value="activity" className="space-y-4 focus:outline-none">
                            {activities.length > 0 ? (
                                activities.map((item, i) => (
                                    <Link href={item.type === 'post' ? `/forum/${item.id}` : `/forum/${item.referenceId}`} key={i}>
                                        <div className="group relative flex items-start gap-5 p-5 rounded-[20px] bg-[#090909] border border-zinc-900 hover:border-zinc-800 hover:bg-[#0c0c0c] transition-all duration-300">
                        ...
                        <TabsContent value="postings" className="space-y-4 focus:outline-none">
                            {recentPosts && recentPosts.length > 0 ? (
                                recentPosts.map((post: any) => (
                                    <Link href={`/forum/${post.id}`} key={post.id}>
                                        <div className="group relative flex items-center gap-5 p-5 rounded-[20px] bg-[#090909] border border-zinc-900 hover:border-zinc-800 hover:bg-[#0c0c0c] transition-all duration-300">
